<?php namespace Laragun\Mailgun\Mailgun;
...
class Message
{
	 ...
	 * @param  string $email
	 * @param  string $name
	 * @return \Laragun\Mailgun\Mailgun\Message
	 */
	...
	{
		$replyTo = Config::get('laragun.mailgun.reply_to');
		if ($replyTo) {
	...
	{
		if (Config::get('laragun.mailgun.force_from_address')) {
	...
	{
		if (Config::get('laragun.mailgun.testmode')) {
	...
	{
		$catchAllMail = Config::get('laragun.mailgun.catch_all');
		if ($catchAllMail) {
